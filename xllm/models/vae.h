...
class VAEEncoderImpl : public torch::nn::Module {
 public:
  VAEEncoderImpl(const ModelArgs& args,
...
class VAEDecoderImpl : public torch::nn::Module {
 public:
  VAEDecoderImpl(const ModelArgs& args,
                 const QuantArgs& quant_args,
                 const ParallelArgs& parallel_args,
                 const torch::TensorOptions& options) {
...
class VAEImpl : public torch::nn::Module {
 public:
  VAEImpl(const ModelArgs& args,
          const QuantArgs& quant_args,
          const ParallelArgs& parallel_args,
          const torch::TensorOptions& options,
          torch::Device device,
          torch::ScalarType dtype)
      : args_(args), device_(device), dtype_(dtype) {
    encoder_ = register_module(
        "encoder", VAEEncoder(args, quant_args, parallel_args, options));
    decoder_ = register_module(
        "decoder", VAEDecoder(args, quant_args, parallel_args, options));
    if (args.use_quant_conv()) {
      quant_conv_ = register_module(
          "quant_conv",
          torch::nn::Conv2d(torch::nn::Conv2dOptions(
              2 * args.latent_channels(), 2 * args.latent_channels(), 1)));
    }
    if (args.use_post_quant_conv()) {
      post_quant_conv_ = register_module(
          "post_quant_conv",
          torch::nn::Conv2d(torch::nn::Conv2dOptions(
              args.latent_channels(), args.latent_channels(), 1)));
    }
    encoder_->to(dtype_);
    decoder_->to(dtype_);
    if (args.use_quant_conv()) {
  ...
  DecoderOutput decode_(const torch::Tensor& latents) {
    torch::Tensor processed_latents = latents;  // 创建可修改的副本
  ...
  torch::Tensor forward(const torch::Tensor& tokens,
    ...
    torch::Tensor sample =
        torch::randn({batch_size, num_channels_latents, height, width},  // 形状
                     torch::dtype(torch::kFloat32)
                         .device(device_)  // 数据类型+设备（合并指定）
        );
