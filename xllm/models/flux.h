...
torch::Tensor randn_tensor(
  ...
  //     // 生成器在CPU但目标设备不是CPU或MPS时，在CPU上生成再移动
...
class FluxPipelineImpl : public torch::nn::Module {
 ...
 public:
  FluxPipelineImpl(const ModelArgs& args,
                   const QuantArgs& quant_args,
                   const ParallelArgs& parallel_args,
                   const torch::TensorOptions& options) {
    vae_scale_factor_ = 1 << (args.block_out_channels().size() - 1);
    _execution_device = options.device();
    ...
    vae_shift_factor_ = args.shift_factor();
    vae_scaling_factor_ = args.scale_factor();
    default_sample_size_ = 128;
    tokenizer_max_length_ = 77;  // TODO: 需要从配置中获取
    ...
    vae_ = VAE(args,
               ...
               _execution_device,
               _execution_dtype);
    transformer_ = DiTModel(args,
                            quant_args,
                            parallel_args,
                            options,
                            _execution_device,
                            _execution_dtype);
    t5_ = T5EncoderModel(args,
                         quant_args,
                         parallel_args,
                         options,
                         _execution_device,
                         _execution_dtype);
    clip_text_model_ = CLIPTextModel(args,
                                     quant_args,
                                     parallel_args,
                                     options,
                                     _execution_device,
                                     _execution_dtype);
    scheduler_ = FluxScheduler(args, quant_args, parallel_args, options);
    // register modules
  ...
  FluxPipelineOutput forward_(
    ...
    if (transformer_->guidance_embeds()) {
      ...
      // 合并dtype和device到同一个TensorOptions
      torch::TensorOptions options =
          torch::dtype(torch::kFloat32).device(device);

      // 与Python代码对齐：torch.full([1], guidance_scale, device=device,
      // dtype=torch.float32)
      guidance = torch::full(at::IntArrayRef({1}), guidance_scale, options);

      // 与Python代码对齐：guidance.expand(latents.shape[0])
      // 直接使用prepared_latents的第一个维度大小进行展开
    ...
    for (int64_t i = 0; i < timesteps.numel(); ++i) {
      ...
      if (do_true_cfg) {
        ...
        negative_noise_pred.reset();  // 显式释放负向预测张量
      }
      // 更新潜变量
      auto prev_latents = scheduler_->step(noise_pred, t, prepared_latents);
      // 替换潜变量并释放旧值（修正结构体初始化错误）
      prepared_latents = prev_latents.prev_sample.detach();  // 切断计算图
      std::vector<torch::Tensor> tensors = {prepared_latents, noise_pred};
      noise_pred.reset();
      // 使用默认构造的空张量重置结构体成员，避免无参构造函数错误
