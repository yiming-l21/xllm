...
void ModelRegistry::register_diffusionmm_factory(const std::string& name,
                                                 DiffusionMMFactory factory) {
  ModelRegistry* instance = get_instance();

  if (instance->model_registry_[name].diffusion_mm_factory != nullptr) {
    LOG(WARNING) << "causal flux factory for " << name << "already registered.";
  } else {
    instance->model_registry_[name].diffusion_mm_factory = factory;
  }
}
...
DiffusionMMFactory ModelRegistry::get_diffusionmm_factory(
    const std::string& name) {
  ModelRegistry* instance = get_instance();

  return instance->model_registry_[name].diffusion_mm_factory;
}
...
std::unique_ptr<DiffusionMM> create_mm_model(const Context& context) {
  // get the factory function for the model type from model registry
  auto factory = ModelRegistry::get_diffusionmm_factory(
      context.get_model_args().model_type());
  if (factory) {
    return factory(context);
  }
