...
class DiTModelImpl : public torch::nn::Module {
 public:
  DiTModelImpl(const ModelArgs& args,
    ...
    flux_transformer_2d_model_ =
        register_module("flux_transformer_2d_model",
                        FluxTransformer2DModel(args.dit_patch_size(),
                                               args.dit_in_channels(),
                                               args.dit_num_layers(),
                                               args.dit_num_single_layers(),
                                               args.dit_attention_head_dim(),
                                               args.dit_num_attention_heads(),
                                               args.dit_joint_attention_dim(),
                                               args.dit_pooled_projection_dim(),
                                               args.dit_guidance_embeds(),
                                               args.dit_axes_dims_rope(),
                                               device_,
                                               dtype_));
  ...
  torch::Tensor forward(const torch::Tensor& hidden_states_input,
    ...
    // 调用 FluxTransformer2DModel 的 forward 方法
    torch::Tensor output =
  ...
  torch::Tensor forward(const torch::Tensor& tokens,
    ...
    auto output = forward(hidden_states,
                          ...
                          txt_ids,
                          guidance);
    auto bytes = torch::pickle_save(output.cpu());
    std::string output_path = std::string(OUTPUT_DIR) + "output.pkl";
  ...
  void load_model(std::unique_ptr<ModelLoader> loader) {
    // 这里可以实现加载模型的逻辑
    // 例如：loader->load(flux_transformer_2d_model_);
