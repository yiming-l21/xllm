 ...
 public:
  T5DenseActDenseImpl(const ModelContext& context) {
    ...
    dropout_ = register_module(
        "dropout", torch::nn::Dropout(model_args.t5_dropout_rate()));
...
class T5AttentionImpl : public torch::nn::Module {
 public:
  T5AttentionImpl(const ModelContext& context,
    ...
    dropout_ = model_args.t5_dropout_rate();
    ...
    dropout_layer_ = register_module(
        "dropout", torch::nn::Dropout(model_args.t5_dropout_rate()));
  }
 ...
 public:
  ...
  torch::Tensor forward(const torch::Tensor& input_ids) {
    ...
    if (attention_mask.has_value()) {
      causal_mask = attention_mask.value().unsqueeze(1).unsqueeze(1);
      causal_mask = (1.0 - causal_mask) * std::numeric_limits<float>::lowest();
      causal_mask = causal_mask.to(options);
    } else {
