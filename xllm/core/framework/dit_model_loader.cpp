...
DITSubfolderLoader::DITSubfolderLoader(const std::string& folder_path,
  ...
  CHECK(!model_weights_files_.empty())
      << "Failed to find model weights files in " << folder_path;
...
bool DITSubfolderLoader::load_args(const std::string& model_weights_path) {
  ...
  if (std::filesystem::exists(tokenizer_config_path)) {
    LOG(INFO) << "Load Tokenizer Args from" << component_name_
    ...
    LOG(WARNING) << "No tokenizer_config.json found in " << component_name_;
...
bool DITSubfolderLoader::load_model_args(
  ...
  if (has_safetensors) {
    LOG(INFO) << "Found safetensors files in " << component_name_
              << ", loading config.json";
    ...
    LOG(INFO) << "No safetensors found in " << component_name_
              << ", searching for any JSON config files.";
    std::vector<std::filesystem::path> json_file_paths;
    for (const auto& entry :
         std::filesystem::directory_iterator(model_weights_path)) {
      LOG(INFO) << "Found file: " << entry.is_regular_file()
                << ", path: " << entry.path()
                << ", ext: " << entry.path().extension();
    ...
    for (const auto& json_file : json_file_paths) {
      JsonReader reader;
      if (!reader.parse(json_file.string())) {
