...
std::optional<ForwardOutput> DiTWorker::step(const ForwardInput& inputs) {
#if defined(USE_NPU)
...
// TODO(mlu): implement mlu set device
  ...
  auto input_params = inputs.dit_input_params.to(device_, dtype_);
  auto generation_params = inputs.dit_generation_params;
...
ForwardInput DiTWorker::prepare_inputs(Batch& batch) {
