...
NpuExecutorImpl::NpuExecutorImpl(DiffusionMM* model,
                                 const ModelArgs& args,
                                 const torch::Device& device,
                                 const runtime::Options& options)
    : mm_model_(model), args_(args), device_(device), options_(options) {}
...
torch::Tensor NpuExecutorImpl::run(const torch::Tensor& tokens,
  ...
  } else if (mm_model_ != nullptr) {
    return mm_model_->forward(tokens, positions, kv_caches, params);
  } else {
