...
NpuExecutorImpl::NpuExecutorImpl(CausalFLUX* model,
                                 const ModelArgs& args,
                                 const torch::Device& device,
                                 const runtime::Options& options)
    : flux_model_(model), args_(args), device_(device), options_(options) {}
...
torch::Tensor NpuExecutorImpl::run(const torch::Tensor& tokens,
  ...
  } else if (flux_model_ != nullptr) {
    return flux_model_->forward(tokens, positions, kv_caches, params);
  } else {
    LOG(FATAL) << "No model is set in NpuExecutorImpl.";
  }
}
