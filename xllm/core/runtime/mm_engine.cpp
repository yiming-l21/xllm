...
MMEngine::MMEngine(const runtime::Options& options,
...
Engine::KVCacheCapacity MMEngine::estimate_kv_cache_capacity() {
...
bool MMEngine::allocate_kv_cache(const Engine::KVCacheCapacity& kv_cache_cap) {
...
bool MMEngine::pull_kv_blocks(const int32_t src_dp_size,
...
MMEngine::load_kv_blocks_from_store_async(
...
void MMEngine::get_device_info(std::vector<std::string>& device_ips,
...
void MMEngine::get_cache_info(std::vector<uint64_t>& cluster_ids,
...
bool MMEngine::link_cluster(const std::vector<uint64_t>& cluster_ids,
...
bool MMEngine::unlink_cluster(const std::vector<uint64_t>& cluster_ids,
                              const std::vector<std::string>& addrs,
                              const std::vector<std::string>& device_ips,
                              const std::vector<uint16_t>& ports,
                              const int32_t src_dp_size) {
...
ForwardOutput MMEngine::step(std::vector<Batch>& batch) {
  ...
  auto results = folly::collectAll(futures).get();
  LOG(INFO) << "After step async,in MMEngine::step";
...
void MMEngine::update_last_step_result(std::vector<Batch>& last_batch) {
  auto dp_size = options_.dp_size();
  auto worker_clients_num = worker_clients_.size();
  auto dp_local_tp_size = worker_clients_num / dp_size;
...
std::vector<int64_t> MMEngine::get_active_activation_memory() const {
...
void MMEngine::setup_workers(const runtime::Options& options) {
  if (!dist_manager_) {
...
void MMEngine::process_eplb_data(
