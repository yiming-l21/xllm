...
namespace xllm {
MMMaster::MMMaster(const Options& options) : Master(options, EngineType::MM) {
  LOG(INFO) << "MM Engine initialized. in MM_master.cpp";
  CHECK(engine_->init());
...
void MMMaster::handle_batch_request(std::vector<std::string> prompts,
...
void MMMaster::handle_batch_request(
...
void MMMaster::handle_request(std::string prompt,
  ...
  threadpool_->schedule([this,
    ...
    LOG(INFO) << "in MM_master.cpp, after generate_request, before "
...
void MMMaster::handle_request(std::vector<Message> messages,
                              std::optional<std::vector<int>> prompt_tokens,
                              RequestParams sp,
                              std::optional<Call*> call,
                              OutputCallback callback) {
  LOG(INFO) << "in MM_master.cpp, into handle_request with messages";
  scheduler_->incr_pending_requests(1);
  auto cb = [callback = std::move(callback)](const RequestOutput& output) {
    output.log_request_status();
    return callback(output);
...
void MMMaster::run() {
  ...
  if (already_running) {
    LOG(WARNING) << "MMMaster is already running.";
    return;
  }
  ...
  loop_thread_ = std::thread([this]() {
    ...
    while (!stoped_.load(std::memory_order_relaxed)) {
      LOG(INFO) << "MMMaster loop step start.";
      scheduler_->step(timeout);
      LOG(INFO) << "MMMaster loop step done."
...
std::shared_ptr<Request> MMMaster::generate_request(
    std::string prompt,
    std::optional<std::vector<int>> prompt_tokens,
...
void MMMaster::get_cache_info(std::vector<uint64_t>& cluster_ids,
...
bool MMMaster::link_cluster(const std::vector<uint64_t>& cluster_ids,
...
bool MMMaster::unlink_cluster(const std::vector<uint64_t>& cluster_ids,
                              const std::vector<std::string>& addrs,
                              const std::vector<std::string>& device_ips,
                              const std::vector<uint16_t>& ports,
                              const int32_t dp_size) {
...
Tokenizer* MMMaster::get_tls_tokenizer() {
