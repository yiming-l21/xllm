...
MMWorkerImpl::MMWorkerImpl(const ParallelArgs& parallel_args,
                           const torch::Device& device,
                           const runtime::Options& options)
    : WorkerImpl(parallel_args, device, options) {}

bool MMWorkerImpl::init_model(torch::ScalarType dtype,
                              const ModelArgs& model_args,
                              const QuantArgs& quant_args) {
  CHECK(mm_model_ == nullptr) << "Model is already initialized.";
#if defined(USE_NPU)
  ...
  mm_model_ = create_mm_model(context_);

  // Dont find model in causal models
  CHECK(mm_model_ != nullptr) << "Failed to create model.";
  model_executor_ = std::make_unique<Executor>(
      mm_model_.get(), model_args, device_, options_);
  return true;
}

std::optional<ForwardOutput> MMWorkerImpl::step(const ForwardInput& inputs) {
  LOG(INFO) << "MMWorkerImpl::step called. in mm_worker_impl.cpp";
#if defined(USE_NPU)
  ...
  LOG(INFO) << "MMWorkerImpl::step done. in mm_worker_impl.cpp";
  return output;
}
