...
bool send_result_to_client_brpc(std::shared_ptr<ImageGenerationCall> call,
  ...
  // TODO fill the response according to req_output for image generation api
...
void ImageGenerationServiceImpl::process_async_impl(
  ...
  auto& input = rpc_request.prompt();
  LOG(INFO) << "Received image generation request, model: " << model
